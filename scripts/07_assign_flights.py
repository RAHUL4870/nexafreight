...
def load_catalog(path: str | None = None) -> dict[str, list[dict[str, Any]]]:
    candidates = [
        ...
        _REPO_ROOT / "Winter_Soldiers-main" / "Winter_Soldiers-main" / "data" / "raw" / "flights" / "flight_catalog.json",
    ]
    for c in candidates:
        p = Path(c)
        if p.is_file():
...
async def amain(args: argparse.Namespace) -> int:
    ...
    try:
        async with engine.begin() as conn:
            ...
            log.info("Prepared flight assignments for %d air legs across %d unique flight numbers", len(assignments), len(used_flights))
            ...
            log.info("Successfully assigned %d air legs to %d flight numbers (provenance=%s, source=%s)", len(assignments), len(used_flights), PROVENANCE, SOURCE)
