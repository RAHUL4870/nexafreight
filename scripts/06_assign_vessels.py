...
def load_catalog(path: str | None = None) -> dict[str, list[dict[str, Any]]]:
    candidates = [
        ...
        _REPO_ROOT / "Winter_Soldiers-main" / "Winter_Soldiers-main" / "data" / "raw" / "vessels" / "vessel_catalog.json",
    ]
    for c in candidates:
        p = Path(c)
        if p.is_file():
...
async def amain(args: argparse.Namespace) -> int:
    ...
    try:
        async with engine.begin() as conn:
                    ...
                    stmt = (
                        ...
                        .on_conflict_do_update(
                            ...
                            set_={
                                ...
                                "typical_lanes_json": sqlite_insert(vessels_tbl).excluded.typical_lanes_json,
                                "updated_at": now,
                            },
                        )
                    )
            ...
            loc_country: dict[int, str] = {row[0]: str(row[1] or "US").upper() for row in res.fetchall()}

            res = await conn.execute(
                select(legs_tbl.c["id"], legs_tbl.c["origin_id"], legs_tbl.c["destination_id"])
            ...
            log.info("Prepared vessel assignments for %d sea legs across %d unique vessels", len(assignments), len(used_mmsis))
            ...
            for i in range(0, len(assignments), batch_size):
                ...
                for leg_id, vid in chunk:
                    await conn.execute(
                        ...
                        .values(vessel_id=vid)
                    )

            log.info("Successfully assigned %d sea legs to %d vessels (provenance=%s, source=%s)", len(assignments), len(used_mmsis), PROVENANCE, SOURCE)
