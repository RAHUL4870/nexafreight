...
from sqlalchemy import Boolean, Column, DateTime, Float, ForeignKey, Integer, MetaData, String, Table, event, select
...
def resolve_dataco_file(user_path: str | None) -> Path:
    candidates = [
        ...
        _REPO_ROOT / "Winter_Soldiers-main" / "Winter_Soldiers-main" / "data" / "raw" / "dataco" / "DataCoSupplyChainDataset.csv",
        _REPO_ROOT / "Datasets" / "Primary Dataset — DataCo Smart Supply Chain" / "DataCoSupplyChainDataset.csv",
    ]
...
def map_cargo_class(category: str) -> str:
    ...
    if any(k in c for k in ("clean", "chem", "paint", "battery", "hazard", "solvent", "flamm")):
        return "HAZMAT"
    if any(k in c for k in ("meat", "fruit", "veg", "dairy", "bake", "frozen", "fish", "food", "perish")):
        return "REFRIGERATED"
...
def _resolve_tables():
    try:
        ...
        orders_tbl = Table(
            ...
            Column("shipment_id", String(36), ForeignKey("shipments.id", ondelete="SET NULL"), nullable=True, index=True),
        ...
        async with engine.begin() as conn:
            ...
            for i in range(0, len(orders_data), batch_size):
                ...
                if (i // batch_size) % 10 == 0 or i + batch_size >= len(orders_data):
                    log.info("Upserted orders [%d..%d] of %d", i + 1, min(i + batch_size, len(orders_data)), len(orders_data))
            ...
            affected_order_ids = [order_num_to_id[o["order_number"]] for o in orders_data if o["order_number"] in order_num_to_id]
            if affected_order_ids:
                for i in range(0, len(affected_order_ids), batch_size):
                    sub_ids = affected_order_ids[i : i + batch_size]
                    await conn.execute(order_items_tbl.delete().where(order_items_tbl.c["order_id"].in_(sub_ids)))
            ...
            for i in range(0, len(item_values), batch_size):
                ...
                if (i // batch_size) % 20 == 0 or i + batch_size >= len(item_values):
                    log.info("Inserted items [%d..%d] of %d", i + 1, min(i + batch_size, len(item_values)), len(item_values))
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Stop after N CSV rows (0 = process all).",
    )
    p.add_argument(
        "--batch-size",
        type=int,
...
async def amain(args: argparse.Namespace) -> int:
    ...
    log.info(
        "DataCo ingestion complete: %d orders and %d order_items written (provenance=%s, source=%s)",
        len(orders_list),
        len(order_items_list),
        PROVENANCE,
        SOURCE,
