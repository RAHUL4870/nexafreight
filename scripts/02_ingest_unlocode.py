...
_ICD_KEYWORDS = (
    "icd", "inland container", "container depot", "dry port", "depot",
)
_WAREHOUSE_KEYWORDS = (
    "warehouse", "distribution center", "distribution centre", "logistics hub",
    "logistics park", "freight village", "cfs", "container freight",
)
...
def resolve_input_files(input_path_str: str) -> list[Path]:
    ...
    candidates = [
        ...
        _REPO_ROOT / "Winter_Soldiers-main" / "Winter_Soldiers-main" / "data" / "raw" / "unlocode" / "unlocode.csv",
        _REPO_ROOT / "Datasets" / "UNLOCODE — Location Codes for Ports, Airports, and ICDs",
    ]
    ...
    for f in files:
        ...
        for enc in ("latin-1", "utf-8-sig", "cp1252", "utf-8"):
            try:
                ...
                has_header = any(c.lower() in ("locode", "country", "name", "coordinates") for c in sample_df.columns)
    ...
    try:
        async with engine.begin() as conn:
            ...
            for i in range(0, len(unique_rows), batch_size):
                ...
                log.info("Upserted batch [%d..%d] of %d rows", i + 1, min(i + batch_size, len(unique_rows)), len(unique_rows))
    finally:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Stop after N rows (0 = process all).",
    )
    p.add_argument(
        "--batch-size",
        type=int,
