...
from sqlalchemy import Column, Date, DateTime, Float, ForeignKey, Integer, MetaData, String, Table, UniqueConstraint, event, select
...
class DailyStat:
    __slots__ = ("port_identifier", "stat_date", "vessel_count", "rolling_90d_avg", "congestion_index")
...
def get_activity_file(user_path: str | None) -> Path:
    cands = [
        ...
        _REPO_ROOT / "Winter_Soldiers-main" / "Winter_Soldiers-main" / "data" / "raw" / "port_activity" / "port_activity.csv",
        _REPO_ROOT / "Datasets" / "IMF Daily Port Activity" / "Daily_Port_Activity_Data_and_Trade_Estimates.csv",
    ]
...
def get_performance_file(user_path: str | None) -> Path:
    cands = [
        ...
        _REPO_ROOT / "Winter_Soldiers-main" / "Winter_Soldiers-main" / "data" / "raw" / "port_performance" / "maritime_port_performance.csv",
        _REPO_ROOT / "Datasets" / "Maritime Port Performance Dataset" / "Maritime Port Performance Project Dataset.csv",
    ]
...
def _load_activity_data(
    ...
    if "portname" in header_df.columns:
        ...
        port_col = next((c for c in header_df.columns if "port" in c.lower() and "id" not in c.lower()), "portname")
    if "portcalls" in header_df.columns:
        calls_col = "portcalls"
    else:
        calls_col = next((c for c in header_df.columns if "calls" in c.lower() or "count" in c.lower()), "portcalls")

    log.info("Using columns: date='%s', port='%s', vessel_calls='%s'", date_col, port_col, calls_col)
...
def build_port_matcher(db_ports: list[tuple[int, str, str, str]], wpi_file: Path | None):
    ...
    for loc_id, locode, name, cc in db_ports:
...
def compute_congestion(daily_stats: Sequence[DailyStat]) -> list[DailyStat]:
    ...
    for p_name, rows in by_port.items():
...
def _resolve_tables():
    try:
        ...
        port_tbl = Table(
            ...
            Column("location_id", Integer, ForeignKey("locations.id", ondelete="RESTRICT"), unique=True, nullable=False),
...
async def populate_database(
    ...
    try:
        async with engine.begin() as conn:
            ...
            log.info("Inserting %d daily congestion snapshots into 'port_daily_stats'...", len(daily_items))
            ...
            for i in range(0, len(daily_items), batch_size):
                ...
                if (i // batch_size) % 10 == 0 or i + batch_size >= len(daily_items):
                    log.info("Upserted daily stats [%d..%d] of %d", i + 1, min(i + batch_size, len(daily_items)), len(daily_items))
...
async def amain(args: argparse.Namespace) -> int:
    ...
    try:
        async with engine.connect() as conn:
            ...
            res = await conn.execute(
                select(loc_tbl.c["id"], loc_tbl.c["locode"], loc_tbl.c["name"], loc_tbl.c["country_code"]).where(
                    loc_tbl.c["location_type"] == "PORT"
                )
            )
            db_ports = res.fetchall()
    finally:
        await engine.dispose()
    ...
    matched_location_ids: set[int] = {port_name_to_loc[r.port_identifier][0] for r in computed_activity}
