...
export type RequestTransport = (url: string, opts: RequestOptions) => Promise<RawResponse>;
let activeTransport: RequestTransport = request;
export function setRequestTransport(t: RequestTransport | null) {
  activeTransport = t || request;
}
...
export async function httpText(url: string, opts: RequestOptions = {}): Promise<string> {
  return (await activeTransport(url, opts)).body;
}
...
export async function httpConditional(
  ...
  const res = await activeTransport(url, { ...opts, headers: { ...opts.headers, ...conditional } });
