// Sanctions screening removed — OFAC SDN data source discontinued in this build.
...
export interface WalletIntel {
  ...
  sanctions: { screened: boolean; hit: boolean; entries: any[] };
...
export async function analyseAddress(addressRaw: string, chainOverride?: Chain): Promise<WalletIntel> {
  ...
  return {
    ...
    sanctions: { screened: false, hit: false, entries: [] },
    risk: { ...risk, factors },
    labels: raw.labels,
    tokens: raw.tokens,
    sources: [...raw.sources, 'CoinGecko'],
    partial: raw.partial,
  };
}
