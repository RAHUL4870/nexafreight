...
describe('skyline resolution', () => {
  it('resolves the Yubatake page to a playable YouTube id', async () => {
    let mockFetch: any;
    if (process.env.RUN_LIVE_TESTS !== '1') {
      mockFetch = (globalThis as any).fetch = async (url: string) => {
    ...
    try {
      ...
      // restore if mocked
      if (mockFetch) delete (globalThis as any).fetch;
    }
  });
});
